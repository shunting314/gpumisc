...
from triton_runner.runner import run_py_kernel, run_ttir_kernel, run_ttgir_kernel, run_llir_kernel, run_ptx_kernel, run_cu_kernel, run_cubin_kernel
from os import path
import torch
import triton
...
class TestRunner(unittest.TestCase):
    ...
    def test_llir_sin(self):
        run_llir_kernel(
            llir=f"{path.dirname(__file__)}/../triton_runner/sin.llir",
            kernel_name="sin_kernel_0d1d2d",
            signature={0: "*f32", 1: "*f32", 2: "i32"},
            grid_x=1,
            args=lambda: (
    ...
    def test_ptx_sin(self):
        run_ptx_kernel(
            ptx=f"{path.dirname(__file__)}/../triton_runner/sin.ptx",
            kernel_name="sin_kernel_0d1d2d",
            signature={0: "*f32", 1: "*f32", 2: "i32"},
            grid_x=1,
            args=lambda: (
    ...
    def test_cu_sin(self):
        run_cu_kernel(
            cu_path=f"{path.dirname(__file__)}/../triton_runner/sin.cu",
            kernel_name="sin_kernel_0d1d2d",
            signature={0: "*f32", 1: "*f32", 2: "i32"},
            grid_x=1,
            args=lambda: (
    ...
    def test_cubin_sin(self):
        run_cubin_kernel(
            cubin=f"{path.dirname(__file__)}/../triton_runner/sin.cubin",
            kernel_name="sin_kernel_0d1d2d",
            signature={0: "*f32", 1: "*f32", 2: "i32"},
            grid_x=1,
            args=lambda: (
